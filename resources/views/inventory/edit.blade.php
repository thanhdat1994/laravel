...
@section('content_header')
    <h1>Update Product Inventories</h1>
@stop
    ...
    <div class="row">
        <div class="col-12">
            <div class="card">
                ...
                <form action="{{ route('inventory.update', $inventory->id) }}" method="POST" class="form-horizontal justify-content-center align-items-center">
                    @method('PATCH')
                    @csrf
                            ...
                            <div class="col-sm-8">
                                <select name="inputProduct" class="form-control" placeholder="Product">
                                    @foreach ($product as $key => $value)
                                        <option value={{ $key }} {{ $key == $inventory->product_id ? 'selected="selected"' : '' }}>{{ $value }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="form-group row">
                            ...
                            <div class="col-sm-8">
                                <div class="input-group date">
                                    <input type="text" id="datepicker" name="inputDate" placeholder="dd/mm/yyyy" class="form-control datetimepicker-input" value="{{ date('d/m/Y', strtotime($inventory->date)) }}"/>
                                </div>
                            </div>
                        </div>
                        <div class="form-group row">
                            ...
                            <div class="col-sm-8">
                                ...
                                <input type="text" class="form-control" name="inputQuantity" placeholder="Quantity" value="{{ number_format($inventory->quantity) }}">
                            </div>
                        </div>
                        <div class="form-group row">
                            <label for="inputPrice" class="col-sm-4 col-form-label text-right">Price</label>
                            <div class="col-sm-8">
                                <input type="text" class="form-control" id="currency" name="inputPrice" placeholder="Price" value="{{ number_format($inventory->price) }}">
                            </div>
                        </div>
                        <div class="form-group row">
                            <label for="inputNote" class="col-sm-4 col-form-label text-right">Note</label>
                            <div class="col-sm-8">
                                <textarea type="text" class="form-control" name="inputNote" placeholder="Note">{{ $inventory->note }}</textarea>
                            </div>
                        </div>
                    </div>
