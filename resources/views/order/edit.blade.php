    ...
    <form action="{{ route('order.update', $order->id) }}" method="POST"
        ...
        <div class="row">
                ...
                <div class="card card-primary">
                    ...
                    <div class="card-body">
                        <div class="form-group row">
                            ...
                            <div class="col-sm-10">
                                <input type="text" class="form-control" name="inputPhoneCustomer" placeholder="Phone" value="{{ $order->customerName->phone }}">
                            </div>
                        </div>
                        <div class="form-group row">
                            <label for="inputNameCustomer" class="col-sm-2 col-form-label text-right">Name</label>
                            <div class="col-sm-10">
                                <input type="text" class="form-control" name="inputNameCustomer" placeholder="Name" value="{{ $order->customerName->name }}">
                            </div>
                        </div>
                        <div class="form-group row">
                            ...
                            <div class="col-sm-10 row mt-2">
                                <div class="form-check mr-5 ml-2">
                                    <input class="form-check-input" type="radio" name="inputSexCustomer" value="0" {{ ($order->customerName->sex == "0")? "checked" : "" }}>
                                    <label class="form-check-label">Male</label>
                                </div>
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="inputSexCustomer" value="1" {{ ($order->customerName->sex == "1")? "checked" : "" }}>
                            ...
                            <div class="col-sm-10">
                                <div class="input-group date">
                                    <input type="text" id="datepicker" name="inputBirthdayCustomer"
                                           placeholder="dd/mm/yyyy" class="form-control datetimepicker-input" value="{{ date('d/m/Y', strtotime($order->customerName->birthday)) }}"/>
                                </div>
                            </div>
                        </div>
                        <div class="form-group row">
                            ...
                            <div class="col-sm-10">
                                <input type="text" class="form-control" name="inputAddressCustomer"
                                       placeholder="Address" value="{{ $order->customerName->address }}">
                            </div>
                        </div>
                        <div class="form-group row">
                            ...
                            <div class="col-sm-10">
                                <select name="inputCustomerClassCustomer" class="form-control" placeholder="Customer Class">
                                    @foreach ($customerClass as $key => $value)
                                        <option value={{ $key }} {{ $key == $order->customerName->customer_class_id ? 'selected="selected"' : '' }}>{{ $value }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="form-group row">
                            ...
                            <div class="col-sm-10">
                                <textarea type="text" class="form-control" name="inputNoteCustomer"
                                          placeholder="Note">{{ $order->customerName->note }}</textarea>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-7">
                <div class="card card-primary">
                    ...
                    <div class="card-body">
                        <div class="form-group row">
                            <label for="inputStatus" class="col-sm-2 col-form-label text-right">Status</label>
                            <div class="col-sm-10">
                                <select name="inputStatus" class="form-control" placeholder="Status">
                                    @foreach (Order::STATUS as $key => $value)
                                        <option value={{ $key }} {{ $key == $order->status ? 'selected="selected"' : '' }}>{{ $value }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                            ...
                            <div class="col-sm-10">
                                <div class="input-group date">
                                    <input type="text" id="dateOrder" name="inputDate" placeholder="dd/mm/yyyy"
                                           class="form-control" value="{{ date('d/m/Y', strtotime($order->date)) }}"/>
                                </div>
                            </div>
                        </div>
                        <div class="form-group row">
                            ...
                            <div class="col-sm-10">
                                <div class="input-group time">
                                    <input type="text" id="timeOrder" name="inputTime"
                                           class="form-control" value="{{ date('H:i:s', strtotime($order->date)) }}"/>
                                </div>
                            </div>
                        </div>
                        <div class="form-group row">
                                    ...
                                    @if (count($order->orderDetail) > 0)
                                        @foreach ($order->orderDetail as $orderDetail)
                                            <tr id="{{ $orderDetail->product_id }}">
                                                ...
                                                <td align="right"><input type="number" class="form-control" name="quantity[]" onchange="sumProduct()" id="quantity_{{ $orderDetail->product_id }}" value="{{ $orderDetail->quantity }}" min="1"/></td>
                                                <td align="right"><input type="text" readonly="true" class="form-control" name="price[]" onchange="sumProduct()" id="price_{{ $orderDetail->product_id }}" value="{{ number_format($orderDetail->price) }}" /></td>
                                                <td align="right" style="vertical-align: middle;"><label name="amount[]" id="amount_{{ $orderDetail->product_id }}">{{ number_format($orderDetail->price * $orderDetail->quantity) }}</label></td>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-10">
                                <input type="text" class="form-control" id="discount" onchange="totalProduct()" name="inputDiscount" value="{{ number_format($order->discount_amount) }}"
        ...
        <div class="row">
            <div class="col-12">
                ...
                <button type="submit" class="btn btn-success float-right"><i class="fas fa-edit"></i> Update
                </button>
            </div>
        </div>
    </form>
